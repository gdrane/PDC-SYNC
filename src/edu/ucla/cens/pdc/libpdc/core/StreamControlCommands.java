...
public class StreamControlCommands extends StreamCommand {
		...
		try {
			...
			if (co == null) {
				...
				return false;
                        ...
                        Log.info("Received the content object for authenticator");
                        Log.info("Received Content Object :" + co);
			...
			Log.info("### REQUESTING STREAMINFO FOR THE STREAM");

			name = publisher.uri.append(ds.app.getAppName()).append(ds.data_stream_id).
					append(Constants.STR_CONTROL).append("stream_info").
					append(config.getRoot());
			
			co = config.getCCNHandle().get(name, SystemConfiguration.LONG_TIMEOUT);
			
			final byte[] streamdata = ds.getTransport().getEncryptor().decryptAsymData(co.
					content());
			StreamInfo si = new StreamInfo();
			si.fromBSON(streamdata);
			ds.getTransport().getEncryptor().setDecryptKey(si.getStreamKey());
	...
	boolean processKey(DataStream ds, ContentName receiver_uri, Interest interest)
	{
		...
		assert key != null;
		assert version != null;
                Log.info("Receiver_uri " + receiver_uri.toString());
